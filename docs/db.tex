...
\begin{figure}[t!]
	...
	\caption{{\em A $\partial\mathbb{B}$ net to illustrate hardening}. The net concatenates a $\partial_{\neg}\!\operatorname{Layer}$ (of width $n$) with a reshaping layer that  outputs two vectors, which get reduced, by a $\partial\!\operatorname{Maj}$ operator,  to 2 soft-bits, one for each class label. A final $\partial\!\operatorname{harden}$ layer ensures the loss is a function of hard bits. When $|{\bf x}|=1$ we choose $n=2$ and therefore the net's weights, once hardened, consume $2$ bits. When $|{\bf x}|=5$ we choose $n=8$ and the weights consume $40$ bits or $5$ bytes.}
	\label{fig:toy-example-architecture}
\end{figure}

First we describe a toy problem to illustrate hard-equivalence. Consider the trivial problem of predicting whether a person wears a $\operatorname{t-shirt}$ (label 0) or a $\operatorname{coat}$ (label 1) conditional on the single feature $\operatorname{outside}$ (0 = False, and 1 = True). The training and test data consists of multiple copies of the data described in table \ref{tab:toy1}.
...
We randomly initialize the network described in figure \ref{fig:toy-example-architecture} and train using the RAdam optimizer \citep{Liu2020On} with softmax cross-entropy loss until training and test accuracies are both $100\%$. The net is hard-equivalent to the discrete program:
...
The learned weights are $w_{1} = False$ and $w_{2} = True$, and therefore the learned discrete program simplifies to:
...
Consider that we introduce 4 additional boolean features: $\operatorname{very-cold}$, $\operatorname{cold}$, $\operatorname{warm}$, and $\operatorname{very-warm}$. The training and test data now consists of examples illustrated in table \ref{tab:toy2}.
...
\end{lstlisting}
We train the $[w_{1}, \dots, w_{40}]$ soft-bit weights as before and then harden to 40 boolean weights. Post-training the learned discrete program then simplifies to:
