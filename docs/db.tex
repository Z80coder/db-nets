...
Say we have a vector of soft-bit inputs ${\bf x}$ and the $i$th element represents the target discrete function (e.g. if our target is $x \wedge y$ then ${\bf x}=[x,y]$ and $i$ is 1 if $x<y$ and $i=2$ otherwise). Now, if we pack only a fraction of the available margin, $|x_{i}-1/2|$, we will not cross the $1/2$ threshold and break the hard-equivalence of the representative bit. The mean soft-bit value, $\bar{\bf x} \in [0,1]$, is just such a gradient-rich fraction. We therefore define 
\begin{equation*}
\begin{aligned}
...
\end{aligned}
\end{equation*}
The packed fraction of the margin increases or decreases with the mean soft-bit value. The available margin tends to zero as the representative bit tends to the hard threshold $1/2$. At the threshold point there is no margin to pack. 
...
Some boolean functions are particularly important for tractable learning.
