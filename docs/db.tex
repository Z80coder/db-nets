...
\begin{figure}[t!]
	...
	\caption{{\em A $\partial\mathbb{B}$ net for MNIST}. The net consists of a $\partial_{\Rightarrow}\!\operatorname{Layer}$ (width 60), a $\partial\!\operatorname{Maj}$ layer, a $\partial_{\neg}\!\operatorname{Layer}$ (width 20), and a final $\partial\!\operatorname{harden}$ operator to generate hard-bits that are summed to produce 10 integer logits.}
...
We measure the accuracy on the final net. Table \ref{tab:mnist-table} compares the $\partial\mathbb{B}$ net against other classifiers \citep{granmo18}. A 2-layer neural network trained on grey-value pixel data performs best. A Tsetlin machine of 40,000 automata each with 256 states (and therefore 40 kb of parameters) trained on binary data achieves $\approx 98.2\%$ accuracy. The $\partial\mathbb{B}$ net with 105,840 32-bit soft-bit weights that harden to booleans (13.23 kb of parameters) trained on binary data achieves $\approx 94.0\%$ accuracy. However, this $\partial\mathbb{B}$ net underfits the training data and therefore we expect better performance from increasing the parameters. However, this causes an OOM error on my GPU. An updated draft will include this experiment.
