...
A $\partial_{\wedge}\!\operatorname{Neuron}$ learns to logically $\wedge$ a subset of the elements of its input vector:
\begin{equation*}
\begin{aligned}
...
\end{aligned}
\end{equation*}
In consequence, a $\partial_{\vee}\!\operatorname{Layer}$ of width $n$ can learn up to $n$ different disjunctions of subsets of its input.
...
\end{aligned}
\end{equation*}
The $\operatorname{harden}$ function is not differentiable and therefore we use the straight-through estimator \citep{DBLP:journals/corr/BengioLC13} during backpropagation. However, by restricting the straight-through estimator to the very final layer we avoid compounding gradient errors to deeper parts of the network.
...
\begin{figure}[t]
	...
	\caption{{\em A $\partial\mathbb{B}$ net for the binary iris problem}.}
