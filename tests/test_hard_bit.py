...
from neurallogic import hard_bit, harden, symbolic_generation
...
def test_activation():
    ...
    for input, expected in test_data:
        check_consistency(
            hard_bit.soft_bit, hard_bit.hard_bit, expected, input[0], input[1]
        )
...
def test_neuron():
    ...
    for input, thresholds, expected in test_data:
        ...
        def soft(thresholds, input):
            return hard_bit.soft_bit_neuron(thresholds, input)

        def hard(thresholds, input):
            return hard_bit.hard_bit_neuron(thresholds, input)
...
def test_layer():
    ...
    for input, thresholds, expected in test_data:
        ...
        def soft(thresholds, input):
            return hard_bit.soft_bit_layer(thresholds, input)

        def hard(thresholds, input):
            return hard_bit.hard_bit_layer(thresholds, input)
